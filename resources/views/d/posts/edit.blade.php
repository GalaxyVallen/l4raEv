  ...
  <div class="col-lg-7">
    <form method="POST" action="/dashboard/posts/{{ $post->slug }}" enctype="multipart/form-data" class="mb-5"> 
      ...
      <div class="mb-3">
        ...
        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" autofocus id="title" value="{{ old('title' , $post->title) }}" required aria-describedby="Create new" onkeyup="generateSlug()">
        <div id="titl" class="form-text">Change my mind?</div>
        @error('title')
      ...
      <div class="mb-3">
        ...
        <input type="text" name="slug" class="form-control @error('slug') is-invalid @enderror" id="slug" value="{{ old('slug' ,$post->slug) }}" aria-describedby="This is slug">
        @error('slug')
          <p class="text-danger mb-0">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="mb-3 border rounded p-1 shadow-sm col-sm-9">
        @if ($post->thumbnail)
        <img src="{{ asset('storage/' . $post->thumbnail) }}" alt="" class="img-fluid rounded img-preview">
        @else
        <img src="https://placehold.co/1200x400?text={{ $post->category->name }}" alt="" class="img-fluid rounded img-preview">
        @endif
      </div>
      <div class="mb-3">
